/*  Space For Unity Editor C# Script (version: 1.6)
    ...
    Version History
     1.6    - New Imphenzia.SpaceForUnity namespace to replace SU_ prefix.
            ...
            - Renamed to SpaceForUnityEditor
            - Inlcuded conditional UnpackPrefab as required by new prefab workflow in Unity 2018.3 and newer
            - Replaced Deprecated EnumMaskField with EnumFlagsField
            - Added checks and automation to change DeepSpace layer name, Camera Culling and Camera Depth for Main Camera.
...
public class SpaceForUnityEditor : EditorWindow
{
    private const int _deepSpaceLayer = 20;
    ...
    [MenuItem("Window/SPACE for Unity")]
    static void Init()
    {
        // Initialize the SPACE for Unity editor window
        EditorWindow.GetWindow(typeof(SpaceForUnityEditor), true, "SPACE for Unity");

        string _modifyCameraNeeded = "";
        if (Camera.main.cullingMask == (Camera.main.cullingMask | (1 << _deepSpaceLayer)))
            _modifyCameraNeeded += "* Set the culling mask for the main camera to exclude the DeepSpace layer (" + _deepSpaceLayer + ").\n";
        if (Camera.main.clearFlags != CameraClearFlags.Depth)
            _modifyCameraNeeded += "* Change the Clear Flags to Depth Only.\n";

        if (_modifyCameraNeeded.Length > 0)
        {
            bool _modifyCameraMain = EditorUtility.DisplayDialog("Note", "Your main camera in the scene will need to be changed for the space scene to look correct.\n\n" +
                "Do you want SPACE for Unity to automatically peform the following changes to your main camera in the scene?\n" +
                 _modifyCameraNeeded + "\n" +
                "See the documentation if you are unsure (Plugins\\Imphenzia\\SpaceForUnity\\_Documentation)"
                , "Ok - Make Changes", "No Thanks");
            if (_modifyCameraMain)
            {
                Camera.main.cullingMask &= ~(1 << _deepSpaceLayer);
                Camera.main.clearFlags = CameraClearFlags.Depth;
            };
        }

        if (!IsDeepSpaceLayerNameCorrect())
        {
            bool _modifyLayerName = EditorUtility.DisplayDialog("Note", "The DeepSpace layer does not have the correct name.\n\n" +
            "Do you want SPACE for Unity to change the name of Layer " + _deepSpaceLayer + " to 'DeepSpace'?\n\n" +
            "See the documentation if you are unsure or if you need to change the DeepSpace layer number (Plugins\\Imphenzia\\SpaceForUnity\\_Documentation)"
            , "Ok - Make Change", "No Thanks");
            if (_modifyLayerName)
            {
                RenameDeepSpaceLayer();
            };
        }
    ...
    void OnEnable()
    {
        ...
        _basePath = AssetDatabase.GetAssetPath(script);
        _basePath = _basePath.Remove(_basePath.Length - "Editor/SpaceForUnityEditor.cs".Length);
    ...
    void OnGUI()
    {
        GUILayout.Label("SPACE for Unity (version: " + _version + ")", EditorStyles.boldLabel);
        ...
        _nebulaBrightness = (NebulaBrightness)EditorGUILayout.EnumFlagsField("Nebula Brightness", _nebulaBrightness);
        _nebulaColors = (NebulaColors)EditorGUILayout.EnumFlagsField("Nebula Colors", _nebulaColors);
        _nebulaStyles = (NebulaStyles)EditorGUILayout.EnumFlagsField("Nebula Styles", _nebulaStyles);
#endif
...
#endif
#if UNITY_2017_3_OR_NEWER
        _planetClimate = (PlanetClimate)EditorGUILayout.EnumFlagsField("Planet Climate:", _planetClimate);
#endif
...
#endif
#if UNITY_2017_3_OR_NEWER
        _planetDistance = (PlanetDistance)EditorGUILayout.EnumFlagsField("Planet Distance:", _planetDistance);
        _planetRotation = (PlanetRotation)EditorGUILayout.EnumFlagsField("Planet Rotation:", _planetRotation);
#endif
...
#endif
#if UNITY_2017_3_OR_NEWER
        _planetMoons = (PlanetMoons)EditorGUILayout.EnumFlagsField("Moons:", _planetMoons);
        _planetMoonDistance = (PlanetMoonDistance)EditorGUILayout.EnumFlagsField("Moon Distance:", _planetMoonDistance);
        _planetMoonOrbitSpeed = (PlanetMoonOrbitSpeed)EditorGUILayout.EnumFlagsField("Moon Orbit Speed:", _planetMoonOrbitSpeed);
#endif
        EditorGUILayout.EndToggleGroup();
    ...
    public void DoCreateSpaceScenePrefab(bool _overwrite)
    {
            ...
            if (_staticStarsEnabled)
                if (TagExists(_tagCamera))
                    if (GameObject.FindGameObjectWithTag(_tagCamera) != null)
                        if (GameObject.FindGameObjectWithTag(_tagCamera).GetComponent<StaticStars>() != null)
        ...
        else
        {
            ...
            _sInstantiated.layer = _deepSpaceLayer;
        }
        ...
        if (_staticStarsEnabled)
        {
            ...
            if (_spaceCamera != null)
            {
                ...
                if (PrefabUtility.GetPrefabInstanceStatus(_sInstantiated) != PrefabInstanceStatus.NotAPrefab)
                    PrefabUtility.UnpackPrefabInstance(_sInstantiated, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
#endif
            ...
            _spaceCamera.gameObject.layer = _deepSpaceLayer;
            _spaceCamera.GetComponent<Camera>().cullingMask = (1 << _deepSpaceLayer);
            _spaceCamera.parent = _sInstantiated.transform;
            // Add the Static Stars component to the space scene camera
            StaticStars _staticStarsComponent = _spaceCamera.gameObject.AddComponent<StaticStars>() as StaticStars;
        ...
        if (_nebulasEnabled)
        {
            ...
            if (_nebulaParent == null)
            {
                ...
                _nebulaParent.layer = _deepSpaceLayer; // Set to DeepSpace scene to avoid it from getting hidden when warping
            ...
            for (int _i = 1; _i <= _nebulaCount; _i++)
            {
                ...
                _nebulaInstantiated.layer = _deepSpaceLayer;
        ...
        if (_galaxiesEnabled)
        {
            ...
            for (int _i = 1; _i <= _numberOfGalaxiesToAdd; _i++)
            {
                ...
                _galaxyInstantiated.layer = _deepSpaceLayer;
...
#if UNITY_2018_3_OR_NEWER
                    if (PrefabUtility.GetPrefabInstanceStatus(_galaxyInstantiated) != PrefabInstanceStatus.NotAPrefab)
                        PrefabUtility.UnpackPrefabInstance(_galaxyInstantiated, PrefabUnpackMode.Completely, InteractionMode.AutomatedAction);
#endif
        ...
        if (_planetsEnabled)
        {
            ...
            for (int _i = 1; _i <= _numberOfPlanetsToAdd; _i++)
            {
                ...
                _planetInstantiated.layer = _deepSpaceLayer;
                ...
                if (_numberOfMoonsToAdd > 0)
                {
                    for (int _n = 1; _n <= _numberOfMoonsToAdd; _n++)
                    {
                        ...
                        _planetMoonInstantiated.layer = _deepSpaceLayer;
                        ...
                        _planetMoonInstantiated.GetComponent<Moon>().orbitSpeed = _moonOrbitSpeed;
                ...
                if (_planetRings != PlanetRings.NEVER)
                {
                    ...
                    if (_addPlanetRings < _planetRingsOdds || _planetRings == PlanetRings.ALWAYS)
                    {
                        ...
                        _planetRingsInstantiated.layer = _deepSpaceLayer;
        ...
        if (_localStarEnabled)
        {
            ...
            _localStarInstantiated.layer = _deepSpaceLayer;
    ...
    static bool IsDeepSpaceLayerNameCorrect()
    {
        ...
        {
            Debug.LogWarning("Can't set layer name.  It's possible the format of the layers and tags data has changed in this version of Unity.");
            Debug.LogWarning("Layers is null: " + (layers == null));
            return true;
        }

        SerializedProperty layerSP = layers.GetArrayElementAtIndex(_deepSpaceLayer);
        if (layerSP.stringValue != "DeepSpace")
        {
            return false;
        }
        return true;
    ...
    static void RenameDeepSpaceLayer()
    {
        SerializedObject tagManager = new SerializedObject(AssetDatabase.LoadAllAssetsAtPath("ProjectSettings/TagManager.asset")[0]);
        ...
        if (layerSP.stringValue != "DeepSpace")
        {
            Debug.Log("Setting Layer Name. Layer " + _deepSpaceLayer + " is now called DeepSapce");
            layerSP.stringValue = "DeepSpace";
        }
