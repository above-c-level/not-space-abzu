...
{
    public class SpaceSceneSwitcherDemoGUI : MonoBehaviour
    {
        ...
        [Tooltip("Assign the asteroid transform in the inspector (drag and drop from e.g. Spaceship/Asteroids)")]
        ...
        void OnGUI()
        {
            ...
            {
                SpaceSceneSwitcher.Switch("SpaceScene1");
            }
            // Create a GUI button to switch to SpaceScene2
            if (GUI.Button(new Rect(10, 40, 180, 25), "Switch to Scene 2 (F2)"))
            {
                SpaceSceneSwitcher.Switch("SpaceScene2");
            }

            // Switch Camera Follow mode
            CameraFollow _cameraFollow = Camera.main.GetComponent<CameraFollow>();
            if (GUI.Button(new Rect(10, 70, 180, 25), "Camera " + _cameraFollow.followMode.ToString()))
            {
                if (_cameraFollow.followMode == CameraFollow.FollowMode.CHASE)
                {
                    // Set following spectator mode
                    _cameraFollow.followMode = CameraFollow.FollowMode.SPECTATOR;
                }
                else
                {
                    // Set chase view mode
                    _cameraFollow.followMode = CameraFollow.FollowMode.CHASE;
                }
            }
            ...
            if (_oldSpaceParticles != _spaceParticles)
            {
                ...
                SpaceSceneSwitcher.SetActive(spaceParticles.gameObject, _spaceParticles);
            ...
            {
                // Use the SU_SpaceSceneSwitcher to set active which takes into consideration compiler specific code
                SpaceSceneSwitcher.SetActive(spaceFog.gameObject, _spaceFog);
                // Remember the value, workaround for how GUI.Toggle works when you can't perform the if statement on the actual toggle
            ...
            {
                // Use the SU_SpaceSceneSwitcher to set active which takes into consideration compiler specific code
                SpaceSceneSwitcher.SetActive(spaceAsteroids.gameObject, _spaceAsteroids);
                // Remember the value, workaround for how GUI.Toggle works when you can't perform the if statement on the actual toggle
